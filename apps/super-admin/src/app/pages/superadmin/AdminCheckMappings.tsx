  ...
  return (
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50" onClick={onClose}>
      <div className="bg-white rounded-xl shadow-xl w-[640px] max-h-[85vh] overflow-y-auto flex flex-col" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between px-5 py-4 border-b border-[#64748B]/20 flex-shrink-0">
          <p className="text-[17px] font-bold text-slate-900">Map a Check to a Control Action</p>
        ...
        <div className="p-5 space-y-4 overflow-y-auto">
          {/* Step 1: provider */}
          <div>
            <label className="block text-[13.5px] font-medium text-slate-600 mb-1">Provider *</label>
          ...
          {providerId && (
            <div>
              <label className="block text-[13.5px] font-medium text-slate-600 mb-1">Check *</label>
              <div className="relative mb-2">
                <Search className="w-3.5 h-3.5 text-slate-400 absolute left-2.5 top-1/2 -translate-y-1/2" />
                <input value={search} onChange={e => setSearch(e.target.value)}
                  placeholder="Search real Prowler checks by ID or title…"
                  className="w-full h-9 pl-8 pr-3 rounded-lg border border-slate-300 text-[14.5px] focus:outline-none focus:border-slate-700" />
              </div>
              {checksLoading ? (
                <div className="py-6 flex justify-center"><Loader2 className="w-4 h-4 animate-spin text-slate-400" /></div>
              ) : (
                <div className="border border-[#64748B]/20 rounded-lg max-h-52 overflow-y-auto divide-y divide-slate-100">
                  {filteredChecks.length === 0 ? (
                    <p className="text-[14px] text-slate-400 text-center py-4">No checks match "{search}"</p>
                    ...
                    <button key={c.check_id} onClick={() => setSelectedCheck(c)}
                      ...
                      <div className="flex items-center gap-2">
                        <p className="text-[14px] font-semibold text-slate-800 truncate flex-1">{c.title}</p>
                        <span className={`text-[11.5px] px-1.5 py-0.5 rounded font-bold uppercase flex-shrink-0 ${SEVERITY_STYLE[c.severity] ?? 'bg-slate-100 text-slate-500'}`}>{c.severity}</span>
                      </div>
                      <p className="text-[12.5px] text-slate-400 font-mono mt-0.5">{c.check_id}</p>
                    </button>
                  ))}
                  {checks.length > 60 && !search && (
                    <p className="text-[12.5px] text-slate-400 text-center py-2">Showing first 60 of {checks.length} — search to narrow down.</p>
                  )}
                </div>
              )}
            </div>
          ...
          {selectedCheck && (
            <>
              <div className="bg-blue-50 border border-blue-100 rounded-lg px-3 py-2 text-[14px] text-blue-800 flex items-center gap-2">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-[13.5px] font-medium text-slate-600 mb-1">Control *</label>
                ...
                <div>
                  <label className="block text-[13.5px] font-medium text-slate-600 mb-1">Action *</label>
                  <select value={actionId} onChange={e => setActionId(e.target.value)} disabled={!controlId}
                    className="w-full h-9 px-3 rounded-lg border border-slate-300 text-[15px] focus:outline-none bg-white focus:border-slate-700 disabled:bg-slate-50">
              ...
              <div>
                <label className="block text-[13.5px] font-medium text-slate-600 mb-1">Status</label>
        ...
        <div className="flex gap-3 px-5 py-4 border-t border-[#64748B]/20 flex-shrink-0">
  ...
  return (
    <div className="max-w-[1400px] mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-[26px] font-bold text-slate-900" style={{ fontFamily: 'Cinzel, serif' }}>Control ↔ Check Mappings</h1>
      ...
      {mappings.length === 0 ? (
        <div className="py-20 text-center border-2 border-dashed border-[#64748B]/20 rounded-xl">
          <ShieldAlert className="w-10 h-10 text-slate-300 mx-auto mb-3" />
          <p className="text-[16px] font-semibold text-slate-500">No mappings yet</p>
          <p className="text-[14px] text-slate-400 mt-1">Map a handful of high-value checks to start — this catalog is meant to grow incrementally, not be complete on day one.</p>
          <button onClick={() => setShowNew(true)} className="mt-4 px-4 py-2 bg-[#1A3E5C] text-white text-[15px] font-semibold rounded-lg">
            Map the First Check
          </button>
        </div>
      ) : (
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-3">
          {mappings.map(m => (
            <div key={m.id} className="flex items-center gap-4 px-5 py-3.5 bg-white border border-[#64748B]/20 rounded-xl shadow-sm shadow-slate-900/[0.03]">
              <div className="w-9 h-9 rounded-lg bg-slate-100 flex items-center justify-center flex-shrink-0">
                <Link2 className="w-4 h-4 text-slate-500" />
              </div>
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 flex-wrap">
                  <p className="text-[15px] font-semibold text-slate-900 font-mono">{m.checkId}</p>
                  <span className="text-[12px] px-1.5 py-0.5 bg-slate-100 text-slate-500 rounded font-medium">{m.provider.displayName}</span>
                  <span className={`text-[12px] px-2 py-0.5 rounded-full font-semibold ${m.status === 'PUBLISHED' ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-100 text-slate-500'}`}>
                    {m.status === 'PUBLISHED' ? 'Published' : 'Draft'}
                  </span>
                  {m.checkSeverity && (
                    <span className={`text-[11.5px] px-1.5 py-0.5 rounded font-bold uppercase ${SEVERITY_STYLE[m.checkSeverity] ?? 'bg-slate-100 text-slate-500'}`}>{m.checkSeverity}</span>
                  )}
                </div>
                <p className="text-[13.5px] text-slate-500 mt-0.5">{m.checkTitle}</p>
                <p className="text-[13px] text-blue-600 mt-0.5 flex items-center gap-1">
                  {m.predefinedAction.control.title} <ChevronRight className="w-3 h-3" /> {m.predefinedAction.title}
                </p>
              </div>
              <div className="flex items-center gap-4 flex-shrink-0">
                <div className="flex items-center gap-2">
                  <span className={`text-[12.5px] font-semibold ${m.isActive ? 'text-emerald-600' : 'text-slate-400'}`}>
