    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50" onClick={onClose}>
      <div className="bg-white rounded-xl shadow-xl w-[480px]" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between px-5 py-4 border-b border-[#64748B]/20">
          <p className="text-[17px] font-bold text-slate-900">{isEdit ? 'Edit' : 'Create'} Control Family</p>
          <button onClick={onClose}><X className="w-4 h-4 text-slate-400" /></button>
        </div>
        <div className="p-5 space-y-4">
          <div>
            <label className="block text-[13.5px] font-medium text-slate-600 mb-1">Name *</label>
          ...
          <div>
            ...
            <textarea value={form.description} onChange={e => up('description', e.target.value)}
              ...
              className="w-full px-3 py-2 rounded-lg border border-slate-300 text-[14.5px] focus:outline-none focus:border-slate-700 resize-none" />
          </div>
        </div>
      ...
      <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50" onClick={onClose}>
        <div className="bg-white rounded-xl shadow-xl w-[600px] max-h-[80vh] flex flex-col" onClick={e => e.stopPropagation()}>
          <div className="flex items-center justify-between px-5 py-4 border-b border-[#64748B]/20 flex-shrink-0">
            <div>
              <p className="text-[17px] font-bold text-slate-900">Assign Controls — {familyName}</p>
              <p className="text-[14px] text-slate-400 mt-0.5">{selected.size} selected</p>
            </div>
            <button onClick={onClose}><X className="w-4 h-4 text-slate-400" /></button>
          </div>
          <div className="p-4 border-b border-[#64748B]/20 flex-shrink-0 flex gap-3">
            <div className="relative flex-1">
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
              <input value={search} onChange={e => setSearch(e.target.value)}
                placeholder="Search controls…"
                className="w-full h-9 pl-9 pr-3 rounded-lg border border-slate-300 text-[15px] focus:outline-none focus:border-slate-700" />
            </div>
            <button onClick={() => setShowCreateControl(true)} className="px-3 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-700 text-[14px] font-semibold rounded-lg flex items-center gap-1.5 transition-colors whitespace-nowrap">
          ...
          {filtered.map((c: any) => (
            <label key={c.id} className="flex items-start gap-3 p-3 rounded-lg hover:bg-slate-50 cursor-pointer">
              ...
              <div className="flex-1">
                <p className="text-[14.5px] font-medium text-slate-800">{c.title}</p>
                <p className="text-[13px] text-slate-400 line-clamp-1">{c.description}</p>
              </div>
              <span className={`text-[12px] px-1.5 py-0.5 rounded font-semibold ${c.status === 'PUBLISHED' ? 'bg-green-100 text-green-700' : 'bg-amber-100 text-amber-600'}`}>
                {c.status}
              </span>
            </label>
          ))}
        ...
        <div className="flex gap-3 px-5 py-4 border-t border-[#64748B]/20 flex-shrink-0">
          ...
          <button onClick={handleSave} disabled={addMut.isPending || removeMut.isPending}
            className="flex-1 py-2 bg-[#1A3E5C] text-white text-[15px] font-semibold rounded-lg hover:bg-[#15324a] flex items-center justify-center gap-2">
  ...
  return (
    <div className="max-w-[1400px] mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-[26px] font-bold text-slate-900" style={{ fontFamily: 'Cinzel, serif' }}>Control Families</h1>
          <p className="text-[14.5px] text-slate-500 mt-1">Group controls by domain for easier regulation building.</p>
        </div>
        <button onClick={() => setShowCreate(true)}
          className="flex items-center gap-2 px-4 py-2 bg-[#1A3E5C] hover:bg-[#15324a] text-white text-[15px] font-semibold rounded-lg">
      ...
      {(families as any[]).length === 0 ? (
        <div className="py-20 text-center border-2 border-dashed border-[#64748B]/20 rounded-xl">
          ...
          <p className="text-[16px] font-semibold text-slate-500">No control families yet</p>
          <p className="text-[14px] text-slate-400 mt-1">Create families to group controls by domain (e.g., Access Management, Data Protection).</p>
          <button onClick={() => setShowCreate(true)} className="mt-4 px-4 py-2 bg-[#1A3E5C] text-white text-[15px] font-semibold rounded-lg">
            Create First Family
          </button>
        </div>
      ) : (
        <div className="space-y-3">
          {(families as any[]).map((family: any) => (
            <div key={family.id} className="bg-white border border-[#64748B]/20 rounded-xl overflow-hidden">
              <div className="flex items-center gap-4 px-5 py-4">
                ...
                <div className="w-10 h-10 rounded-lg bg-slate-50 border border-[#64748B]/20 flex items-center justify-center flex-shrink-0">
                  <Layers className="w-5 h-5 text-slate-400" />
                </div>
                <div className="flex-1">
                  <div className="flex items-center gap-2">
                    <p className="text-[16px] font-bold text-slate-900">{family.name}</p>
                    <span className="text-[12px] px-2 py-0.5 rounded-full font-semibold bg-slate-100 text-slate-600">
                      {family._count.controls} controls
                    </span>
                  </div>
                  {family.description && <p className="text-[14px] text-slate-400 mt-0.5">{family.description}</p>}
                </div>
                <div className="flex gap-2 flex-shrink-0">
                  <button onClick={() => setAssignFor(family)}
                    className="px-3 py-1.5 text-[14px] border border-[#64748B]/20 rounded-lg hover:bg-slate-50 text-slate-600">
                    Manage Controls
                  </button>
                ...
                <div className="border-t border-slate-100 px-5 py-3 space-y-1.5">
                  {family.controls.map((m: any) => (
                    <div key={m.control.id} className="flex items-center gap-3 py-1.5 px-3 rounded-lg hover:bg-slate-50">
                      ...
                      <p className="text-[14.5px] text-slate-700 flex-1">{m.control.title}</p>
                      <span className="text-[12.5px] text-slate-400">{m.control.applicableTo}</span>
                    </div>
                  ))}
                </div>
              )}
              {expanded.has(family.id) && family.controls.length === 0 && (
                <div className="border-t border-slate-100 px-5 py-4 text-center text-[14px] text-slate-400">
