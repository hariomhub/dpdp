    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50" onClick={onClose}>
      <div className="bg-white rounded-xl shadow-xl w-[480px]" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between px-5 py-4 border-b border-[#64748B]/20">
          <p className="text-[17px] font-bold text-slate-900">{isEdit ? 'Edit' : 'Add'} Asset Template</p>
        ...
        <div className="p-5 space-y-3">
          <div>
            ...
            <select value={form.providerId} disabled={isEdit}
              ...
              className="w-full h-9 px-3 rounded-lg border border-slate-300 text-[15px] focus:outline-none bg-white focus:border-slate-700 disabled:bg-slate-50 disabled:text-slate-400">
              <option value="">Select provider…</option>
              {(providers as any[]).map((p: any) => <option key={p.id} value={p.id}>{p.displayName}</option>)}
            </select>
          </div>
          <div>
            <label className="block text-[13.5px] font-medium text-slate-600 mb-1">Cloud Resource Type *</label>
            ...
            <p className="text-[12.5px] text-slate-400 mt-1">The exact ResourceType string from Prowler's check metadata for this provider.</p>
          </div>
          <div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-[13.5px] font-medium text-slate-600 mb-1">Asset Type *</label>
            ...
            <div>
              <label className="block text-[13.5px] font-medium text-slate-600 mb-1">Default Criticality</label>
  ...
  return (
    <div className="max-w-[1400px] mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-[26px] font-bold text-slate-900" style={{ fontFamily: 'Cinzel, serif' }}>Generic Asset Catalog</h1>
        ...
        <div className="flex gap-2 flex-shrink-0">
          <select value={providerFilter} onChange={e => setProviderFilter(e.target.value)}
            className="h-9 px-3 rounded-lg border border-slate-300 text-[14.5px] focus:outline-none bg-white focus:border-slate-700">
      ...
      {filtered.length === 0 ? (
        <div className="py-20 text-center border-2 border-dashed border-[#64748B]/20 rounded-xl">
          ...
          <p className="text-[16px] font-semibold text-slate-500">No asset templates yet</p>
          <p className="text-[14px] text-slate-400 mt-1">Map a provider's real resource type to an asset type to start building the catalog.</p>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-3">
          {filtered.map(t => (
            <div key={t.id} className="flex items-center gap-4 px-5 py-3.5 bg-white border border-[#64748B]/20 rounded-xl shadow-sm shadow-slate-900/[0.03]">
              <div className="w-9 h-9 rounded-lg bg-slate-100 flex items-center justify-center flex-shrink-0">
                <Layers className="w-4 h-4 text-slate-500" />
              </div>
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 flex-wrap">
                  <p className="text-[15.5px] font-semibold text-slate-900">{t.displayName}</p>
                  <span className="text-[12px] px-1.5 py-0.5 bg-slate-100 text-slate-500 rounded font-medium">{t.provider.displayName}</span>
                  <span className={`text-[12px] px-2 py-0.5 rounded-full font-semibold ${t.status === 'PUBLISHED' ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-100 text-slate-500'}`}>
                    {t.status === 'PUBLISHED' ? 'Published' : 'Draft'}
                  </span>
                  <span className={`text-[12px] px-2 py-0.5 rounded-full font-semibold ${CRITICALITY_STYLE[t.defaultCriticality] ?? 'bg-slate-100 text-slate-500'}`}>
                    {t.defaultCriticality}
                  </span>
                </div>
                <p className="text-[13px] text-slate-400 mt-0.5 font-mono">{t.cloudResourceType}</p>
                <p className="text-[13px] text-blue-600 mt-0.5">→ {ASSET_TYPE_LABELS[t.assetType] ?? t.assetType}</p>
              </div>
              <div className="flex items-center gap-4 flex-shrink-0">
                <div className="flex items-center gap-2">
                  <span className={`text-[12.5px] font-semibold ${t.isActive ? 'text-emerald-600' : 'text-slate-400'}`}>
