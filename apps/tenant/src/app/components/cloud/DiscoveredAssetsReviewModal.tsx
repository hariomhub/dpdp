  ...
  return (
    <CloudModal title="Confirm as asset" subtitle={draft.name} onClose={onClose}>
      <div className="space-y-4">
        {needsAssetType && (
          <div className="bg-amber-50 border border-amber-200 text-amber-700 rounded-lg px-3 py-2 text-[14px]">
        ...
        <div>
          ...
          <select
            ...
            className="w-full h-9 px-3 rounded-md bg-white border border-slate-300 text-[15px] focus:outline-none focus:border-[#1A3E5C]"
          >
        ...
        <div>
          ...
          {draft.suggestedAssetType && (
            <p className="text-[13px] text-slate-400 mt-1">Suggested from the scan — change it if it's not right.</p>
          )}
        </div>
      </div>
...
function DraftRow({ draft, onConfirm }: { draft: DiscoveredAssetDraft; onConfirm: () => void }) {
  ...
  return (
    <div className="flex items-center gap-3 px-4 py-3 border border-[#D4AF37]/35 rounded-lg hover:border-slate-300 transition-colors">
      <span className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 ${recognized ? 'bg-[#1A3E5C]/8 text-[#1A3E5C]' : 'bg-slate-100 text-slate-400'}`}>
        {recognized ? <Database className="w-4 h-4" /> : <HelpCircle className="w-4 h-4" />}
      </span>

      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          <p className="text-[15px] font-semibold text-slate-800 truncate">{draft.name}</p>
          {recognized ? (
            <span className="text-[12px] px-1.5 py-0.5 bg-[#1A3E5C]/8 text-[#1A3E5C] rounded font-medium flex-shrink-0">
              {ASSET_TYPE_LABELS[draft.suggestedAssetType!] ?? draft.suggestedAssetType}
            </span>
          ) : (
            <span className="text-[12px] px-1.5 py-0.5 bg-amber-50 text-amber-700 rounded font-medium flex-shrink-0">
              Unrecognized
            </span>
          )}
        </div>
        <div className="flex items-center gap-3 mt-0.5 text-[13px] text-slate-400">
      ...
      <div className="flex items-center gap-2 flex-shrink-0">
        ...
        <button
          onClick={onConfirm}
          className="flex items-center gap-1.5 px-3 py-1.5 bg-emerald-50 text-emerald-700 hover:bg-emerald-100 text-[14px] font-semibold rounded-lg transition-colors"
        >
      ...
      >
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-14 text-center">
            ...
            <h3 className="text-[16px] font-semibold text-slate-700">Nothing to review</h3>
            <p className="text-[14.5px] text-slate-400 mt-1 max-w-xs">
