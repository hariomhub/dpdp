...
function EntraConnectInline() {
  ...
  return (
    <div className="space-y-4">
      {/* ── Status header ── */}
      <div className={`flex items-center justify-between p-4 rounded-xl border ${entraStatus?.connected ? 'bg-green-50 border-green-200' : 'bg-blue-50 border-blue-200'}`}>
        <div className="flex items-center gap-3">
          <div className={`w-8 h-8 rounded-full flex items-center justify-center ${entraStatus?.connected ? 'bg-green-500' : 'bg-blue-500'}`}>
            {entraStatus?.connected ? <CheckCircle2 className="w-4 h-4 text-white" /> : <Shield className="w-4 h-4 text-white" />}
          </div>
          <div>
            <p className={`text-[13px] font-bold ${entraStatus?.connected ? 'text-green-800' : 'text-blue-800'}`}>
              Microsoft Entra ID — {entraStatus?.connected ? 'Connected' : 'Not Connected'}
            </p>
            <p className={`text-[11px] mt-0.5 ${entraStatus?.connected ? 'text-green-600' : 'text-blue-600'}`}>
        ...
        <div className="flex items-center gap-2">
          {entraStatus?.connected && (
            <button type="button" onClick={() => syncMut.mutate()} disabled={syncMut.isPending}
              className="flex items-center gap-1.5 px-3 py-1.5 bg-white border border-green-300 text-green-700 text-[12px] font-medium rounded-lg hover:bg-green-50 transition-colors">
          ...
          {!entraStatus?.connected && (
            <button type="button" onClick={() => setShowForm(!showForm)}
              className="px-4 py-1.5 bg-blue-600 hover:bg-blue-700 text-white text-[12px] font-semibold rounded-lg transition-colors">
      ...
      {showForm && !entraStatus?.connected && (
        <div className="p-4 bg-white border border-slate-200 rounded-xl shadow-sm space-y-3">
          <div>
            <p className="text-[13px] font-bold text-slate-800 mb-0.5">Register an Azure AD App</p>
            ...
            {[
              ...
              <div key={f.key}>
                ...
                <input
                  ...
                  className="w-full h-8 px-2.5 rounded-lg border border-slate-300 text-[11.5px] focus:outline-none focus:border-blue-500 transition-colors" />
              </div>
            ))}
          </div>
          <button type="button" onClick={handleConnect}
            disabled={connectMut.isPending || !form.clientId || !form.azureTenantId}
            className="flex items-center gap-2 px-5 py-2 bg-blue-600 hover:bg-blue-700 disabled:opacity-60 text-white text-[13px] font-semibold rounded-lg transition-colors">
      ...
      {entraStatus?.connected && (
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <p className="text-[12.5px] font-bold text-slate-700">Group → Role Mappings</p>
            <button type="button" onClick={() => setShowAddMapping(!showAddMapping)}
              className="flex items-center gap-1.5 px-3 py-1.5 text-[12px] text-blue-700 bg-blue-50 border border-blue-200 rounded-lg hover:bg-blue-100 transition-colors font-medium">
          ...
          {(mappings as any[]).length > 0 ? (
            <div className="border border-slate-200 rounded-xl overflow-hidden">
              {(mappings as any[]).map((m: any, i: number) => (
                <div key={m.id ?? i} className="flex items-center gap-3 px-4 py-2.5 border-b border-slate-100 last:border-0 bg-white hover:bg-slate-50 text-[12px]">
            ...
            <p className="text-[12px] text-slate-400 italic">No mappings yet. Add one to map Entra groups to roles.</p>
          )}

          {/* Add mapping form */}
          {showAddMapping && (
            <div className="p-4 bg-white border border-blue-200 rounded-xl space-y-3">
              <p className="text-[12.5px] font-bold text-slate-800">Map Entra Group → Role</p>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-[11px] font-semibold text-slate-600 mb-1">Entra Group</label>
                  {(groups as any[]).length > 0 ? (
                    <select value={newMapping.entraGroupId}
                      ...
                      className="w-full h-8 px-2.5 rounded-lg border border-slate-300 text-[11.5px] focus:outline-none focus:border-blue-500">
                ...
                <div>
                  <label className="block text-[11px] font-semibold text-slate-600 mb-1">Map to Role</label>
                ...
                <div className="col-span-2">
                  <label className="block text-[11px] font-semibold text-slate-600 mb-1">Assign to Department (optional)</label>
              ...
              <div className="flex gap-2">
                <button type="button" onClick={() => setShowAddMapping(false)}
                  className="px-4 py-1.5 border border-slate-300 text-[12px] text-slate-600 rounded-lg hover:bg-slate-50">Cancel</button>
                <button type="button" onClick={handleAddMapping}
                  disabled={saveMappingsMut.isPending || !newMapping.entraGroupId}
                  className="flex items-center gap-1.5 px-4 py-1.5 bg-blue-600 hover:bg-blue-700 disabled:opacity-60 text-white text-[12px] font-semibold rounded-lg transition-colors">
    ...
    <div className="space-y-5">
      ...
      <div className="flex bg-slate-100 rounded-xl p-1 gap-1">
        <button type="button" onClick={() => setActiveTab('entra')}
          className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-lg text-[13px] font-semibold transition-all ${activeTab === 'entra' ? 'bg-white shadow-sm text-blue-700' : 'text-slate-500 hover:text-slate-700'}`}>
          <Shield className="w-4 h-4" /> Microsoft Entra ID
        </button>
        <button type="button" onClick={() => setActiveTab('manual')}
          className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-lg text-[13px] font-semibold transition-all ${activeTab === 'manual' ? 'bg-white shadow-sm text-blue-700' : 'text-slate-500 hover:text-slate-700'}`}>
      ...
      {activeTab === 'manual' && (
        <div className="space-y-5">
          <div className="p-4 bg-white border border-slate-200 rounded-xl space-y-4">
            <p className="text-[12.5px] font-bold text-slate-800">Add Team Member</p>
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="colleague@company.com"
                  className="w-full h-9 px-3 rounded-lg border border-slate-300 text-[13px] focus:outline-none focus:border-blue-500" />
              </div>
              <div>
            ...
            <div>
              <label className="block text-[12px] font-medium text-slate-700 mb-1.5">Role</label>
              <div className="flex flex-wrap gap-2">
                {['Compliance Officer', 'IT Admin', 'Internal Auditor', 'External Auditor'].map(r => (
                  <button key={r} type="button" onClick={() => setRole(r)}
                    className={`px-3 py-1.5 rounded-lg text-[12px] font-medium border transition-colors ${role === r ? 'bg-indigo-50 border-indigo-200 text-indigo-700' : 'bg-white border-slate-200 text-slate-600 hover:border-slate-300'}`}>
                    {r}
                  </button>
                ))}
              </div>
            ...
            {needsDepts && displayDepts.length > 0 && (
              <div>
                <div className="flex items-center justify-between mb-2">
                  <label className="block text-[12px] font-medium text-slate-700">Assign to Departments (Optional)</label>
                  <button type="button"
                    onClick={() => {
                ...
                <div className="border border-slate-200 rounded-lg overflow-hidden divide-y divide-slate-100">
                  {displayDepts.map(d => (
                    <label key={d.id} className="flex items-center gap-3 px-3 py-2.5 hover:bg-slate-50 cursor-pointer transition-colors">
                      ...
                      />
                      <span className="text-[12.5px] text-slate-800 font-medium">{d.name}</span>
                    </label>
                  ))}
                </div>
              </div>
            ...
            {needsDepts && displayDepts.length === 0 && (
              <p className="text-[11.5px] text-amber-600 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2">
            ...
            <button type="button" onClick={handleAdd} disabled={!email || !name || !role}
              className="flex items-center gap-2 px-4 py-2 bg-slate-900 text-white rounded-lg text-[13px] font-medium hover:bg-slate-800 transition-colors disabled:opacity-50">
          ...
          {invites.length > 0 && (
            <div className="space-y-2">
              <h4 className="text-[13px] font-bold text-slate-800">Pending Invites ({invites.length})</h4>
              {invites.map(inv => (
                <div key={inv.id} className="flex items-center justify-between p-3 bg-white border border-slate-200 rounded-lg">
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-full bg-indigo-50 flex items-center justify-center">
                      <Mail className="w-4 h-4 text-indigo-600" />
                    </div>
                    <div>
                      <p className="text-[13px] font-bold text-slate-900">{inv.name} <span className="font-normal text-slate-500">({inv.email})</span></p>
                      <div className="flex items-center gap-2 mt-0.5">
                        <span className="text-[11px] font-medium text-indigo-600 bg-indigo-50 px-2 py-0.5 rounded-md">{inv.role}</span>
                        {inv.depts.length > 0 && (
                          <span className="text-[11px] text-slate-500">{inv.depts.join(', ')}</span>
                        )}
                      </div>
                    </div>
                  </div>
