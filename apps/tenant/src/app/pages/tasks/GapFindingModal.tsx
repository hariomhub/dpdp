      ...
      <div className="bg-white rounded-xl shadow-xl w-[480px] max-h-[85vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-4 space-y-3.5">
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5">
              <p className="text-[14.5px] font-semibold text-slate-600">Which action(s) have a gap?</p>
              {actions.length > 1 && (
            ...
            <div className="space-y-1 max-h-28 overflow-y-auto border border-[#D4AF37]/20 rounded-lg p-2">
              {actions.map(a => (
          ...
          {/* Reason codes */}
          <div>
            <p className="text-[14.5px] font-semibold text-slate-600 mb-1.5">Why is this being rejected?</p>
              ...
              {REASON_OPTIONS.map(r => (
                <label key={r.value} className="flex items-center gap-2 px-1.5 py-1 rounded hover:bg-slate-50 cursor-pointer">
                  <input type="checkbox" checked={reasonCodes.has(r.value)} onChange={() => toggleReason(r.value)}
                    className="accent-red-600" />
                  <span className="text-[14.5px] text-slate-700">{r.label}</span>
                </label>
              ))}
            </div>
            {reasonCodes.has('OTHER') && (
              <input value={otherReason} onChange={e => setOtherReason(e.target.value)}
                ...
                className="mt-1.5 w-full px-3 py-1.5 text-[15px] border border-[#D4AF37]/35 rounded-lg focus:outline-none focus:border-red-400" />
            )}
          </div>

          {/* Remediation */}
          <div>
            <label className="text-[14.5px] font-semibold text-slate-600">What should be done to fix this?</label>
            <textarea rows={3} value={remediation} onChange={e => setRemediation(e.target.value)}
              ...
              className="w-full mt-1 px-3 py-1.5 text-[15px] border border-[#D4AF37]/35 rounded-lg resize-none focus:outline-none focus:border-red-400" />
          </div>

          {/* Supporting files */}
          <div className="space-y-1.5">
            <label className="text-[14.5px] font-semibold text-slate-600">Supporting files (optional)</label>
            <label className="flex items-center gap-2 px-3 py-2 border border-dashed border-slate-300 rounded-lg cursor-pointer hover:border-red-400">
              ...
              <span className="text-[15px] text-slate-500">Attach one or more files showing the gap</span>
        ...
        <div className="flex gap-3 px-5 py-4 border-t border-[#D4AF37]/35">
          ...
          <button onClick={handleSubmit} disabled={!canSubmit || addFindingMut.isPending}
            className="flex-1 py-2 bg-red-600 text-white text-[16px] font-semibold rounded-lg hover:bg-red-700 disabled:opacity-50 flex items-center justify-center gap-2">
