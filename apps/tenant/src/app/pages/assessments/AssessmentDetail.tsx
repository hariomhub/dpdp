...
export function AssessmentDetailPage() {
  ...
  return (
    <div>
      <button onClick={() => navigate('/org/assessments')} className="flex items-center gap-1.5 text-[15px] text-slate-500 hover:text-slate-800 mb-3">
      ...
      <div className="flex items-start justify-between mb-4">
        <div>
          <h1 className="text-[24px] font-bold text-slate-900" style={{ fontFamily: 'Cinzel, serif' }}>{assessment.name}</h1>
          <div className="flex items-center gap-2 mt-1 flex-wrap">
            ...
            {assAssets.map((a: any) => (
              <span key={a.id} className="text-[13px] px-1.5 py-0.5 bg-slate-100 border border-[#D4AF37]/35 text-slate-600 rounded">{a.name}</span>
            ))}
          </div>
        </div>
      ...
      <div className="flex gap-4">
          ...
          {activeTab === 'Controls' && (
            <div className="space-y-2">
              {chapters.map((chapter: any) => (
                <div key={chapter} className="bg-white border border-[#D4AF37]/35 rounded-lg shadow-sm shadow-slate-900/[0.04] overflow-hidden">
                  <div className="px-4 py-2.5 bg-slate-50 border-b border-[#D4AF37]/35">
                    <span className="text-[13px] font-semibold text-slate-500 uppercase tracking-wider">{chapter}</span>
                  </div>
                  {validControls.filter((c: any) => c.chapter === chapter).map((ctrl: any) => (
                    <div key={ctrl.id} className="flex items-center gap-3 px-4 py-3 border-b border-slate-100 last:border-0 hover:bg-slate-50 cursor-pointer transition-colors"
                      ...
                      <span className="flex-1 text-[14.5px] text-slate-700">{ctrl.title}</span>
              ...
              {chapters.length === 0 && (
                <div className="text-center py-12 text-slate-400 text-[15px]">No controls mapped to this assessment.</div>
              )}
            </div>
          )}
            ...
            <div className="space-y-2">
              {assessment.complianceTasks.map((action: any) => (
                <div key={action.id} onClick={() => navigate(`/org/actions/${action.id}`)}
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-[14.5px] font-medium text-slate-800 truncate">{action.title}</p>
                    <p className="text-[13px] text-slate-400">{action.asset?.name} · {action.assignedTo?.name || 'Unassigned'}</p>
                  </div>
                  <StatusChip status={action.status} />
                </div>
              ))}
              {assessment.complianceTasks.length === 0 && (
                <div className="text-center py-12 text-slate-400 text-[15px]">No actions created yet.</div>
              )}
            </div>
          )}
          ...
          {activeTab === 'Audit Trail' && (
            <div className="bg-white border border-[#D4AF37]/35 rounded-lg shadow-sm shadow-slate-900/[0.04] overflow-hidden">
              <div className={`flex gap-3 px-4 py-3 border-b border-slate-100`}>
                <p className="text-[13px] font-mono text-slate-400 w-40 flex-shrink-0">{new Date(assessment.createdAt).toLocaleString()}</p>
                <p className="text-[14.5px] text-slate-700"><strong className="font-semibold">System</strong> <span className="text-slate-500">created this assessment</span></p>
              </div>
              <div className={`flex gap-3 px-4 py-3`}>
                <p className="text-[13px] font-mono text-slate-400 w-40 flex-shrink-0">{new Date(assessment.createdAt).toLocaleString()}</p>
                <p className="text-[14.5px] text-slate-700"><strong className="font-semibold">System</strong> <span className="text-slate-500">mapped {totalValidControls} controls and excluded {assessment.controls.filter((c:any) => c.isExcluded).length}</span></p>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="w-60 flex-shrink-0 space-y-3">
          <Card>
            <h3 className="text-[14px] font-semibold text-slate-700 mb-3">Assessment Scope</h3>
          ...
          <Card>
            <h3 className="text-[14px] font-semibold text-slate-700 mb-2">Timeline</h3>
      ...
      {showCloseModal && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="w-96 bg-white border border-[#D4AF37]/35 rounded-xl shadow-sm shadow-slate-900/[0.04] shadow-2xl p-6">
            <div className="flex items-start gap-3 mb-4">
              <div className="w-9 h-9 rounded-full bg-amber-100 flex items-center justify-center flex-shrink-0">
                <AlertTriangle className="w-5 h-5 text-amber-600" />
              </div>
              <div className="flex-1">
                <h3 className="text-[17px] font-semibold text-slate-900 mb-1">Close Assessment?</h3>
