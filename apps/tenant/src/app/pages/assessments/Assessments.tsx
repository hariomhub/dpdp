  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 gap-3">
        {isLoading ? (
          <div className="col-span-2 text-center py-16 text-slate-500">Loading assessments...</div>
            ...
            <Card key={ass.id} onClick={() => navigate(`/org/assessments/${ass.id}`)}>
              ...
              <div className="flex flex-wrap gap-1 mb-2.5">
                {assAssets.slice(0, 3).map((a: any) => (
                  <span key={a.asset.id} className="text-[10px] px-1.5 py-0.5 bg-slate-100 border border-slate-200 text-slate-600 rounded">{a.asset.name}</span>
                ))}
                {assAssets.length > 3 && (
                  <span className="text-[10px] px-1.5 py-0.5 bg-slate-100 border border-slate-200 text-slate-400 rounded">+{assAssets.length - 3} more</span>
                )}
              </div>
