...
export function PolicyDetailPage() {
  ...
  return (
    <div>
      <button onClick={() => navigate('/org/policies')} className="flex items-center gap-1.5 text-[13px] text-slate-500 hover:text-slate-800 mb-3">
        ...
        <div>
          <div className="flex items-center gap-2 mb-1 flex-wrap">
            ...
            {policy.effectiveDate && (
              <span className="text-[12px] text-slate-400">Effective: {policy.effectiveDate}</span>
            )}
          </div>
          <h1 className="text-[20px] font-bold text-slate-900" style={{ fontFamily: 'Sora, sans-serif' }}>{policy.name}</h1>
          <p className="text-[12px] text-slate-500 mt-0.5">Owner: {policy.owner}</p>
        </div>
      ...
      {activeTab === 'Document' && (
        <div className="space-y-3">
          <Card>
            <h3 className="text-[13px] font-semibold text-slate-800 mb-3">Version History</h3>
            <div className="space-y-2">
              {versions.map((v, i) => (
                <div key={i} className={`flex items-center justify-between p-3 rounded-lg border ${v.status === 'Active' ? 'bg-green-50 border-green-200' : 'bg-slate-50 border-slate-200 opacity-70'}`}>
                  <div className="flex items-center gap-3">
                    <MonoBadge>{v.version}</MonoBadge>
                    <div>
                      <span className={`text-[11px] font-medium ${v.status === 'Active' ? 'text-green-700' : 'text-slate-400'}`}>{v.status}</span>
                      <p className="text-[11px] text-slate-400">{v.author} · {v.date}</p>
                    </div>
                  </div>
            ...
            <p className="text-[11px] text-slate-400 mt-3 italic">Note: Only one version can be Active at a time. Saving a new version automatically archives the previous one.</p>
          </Card>

          <Card>
            <h3 className="text-[13px] font-semibold text-slate-800 mb-3">Policy Content</h3>
            ...
            <tbody>
              {CONTROLS.slice(0, policy.linkedControls).map(ctrl => (
                <tr key={ctrl.id} className="border-b border-slate-100 hover:bg-slate-50 cursor-pointer transition-colors" onClick={() => navigate(`/org/controls/${ctrl.id}`)}>
                  ...
                  <td className="px-4 py-3 text-[12.5px] text-slate-800">{ctrl.title}</td>
        ...
        <div className="space-y-2">
          {ASSETS.slice(0, policy.linkedAssets).map(asset => (
            <div key={asset.id} onClick={() => navigate(`/org/assets/${asset.id}`)}
              ...
              <div>
                <p className="text-[12.5px] font-medium text-slate-800">{asset.name}</p>
        ...
        <div className="bg-white border border-slate-200 rounded-lg overflow-hidden">
          {[
            ...
            <div key={i} className={`flex gap-4 px-4 py-3 ${i < arr.length - 1 ? 'border-b border-slate-100' : ''}`}>
              <p className="text-[11px] font-mono text-slate-400 w-28 flex-shrink-0">{e.time}</p>
