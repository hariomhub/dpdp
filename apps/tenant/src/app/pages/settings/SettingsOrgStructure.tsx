  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-[18px] font-bold text-slate-900" style={{ fontFamily: 'Cinzel, serif' }}>Organization Structure</h2>
          <p className="text-[14px] text-slate-400 mt-0.5">Manage departments, assets, and suppliers. Changes here immediately affect compliance tasks.</p>
        </div>
        <button onClick={() => setModalState({ type: 'addDept' })} className="flex items-center gap-1.5 px-3 py-2 bg-[#1A3E5C] text-white text-[14.5px] font-semibold rounded-lg hover:bg-[#15324a] transition-colors">
      ...
      {depts.length === 0 ? (
        ...
        <div className="space-y-3">
          {depts.map((dept: any) => {
            ...
            return (
              <div key={dept.id} className="border border-[#D4AF37]/35 bg-white rounded-xl overflow-hidden shadow-sm">
                <div className="flex items-center gap-3 px-4 py-3 hover:bg-slate-50 transition-colors">
                  ...
                  <span className="text-[13px] text-slate-400 font-medium">{deptAssets.length} assets · {deptSuppliers.length} suppliers</span>
                  <div className="flex items-center gap-1 ml-2">
                    <button onClick={() => setModalState({ type: 'addAsset', deptId: dept.id })} className="px-2 py-1 bg-green-50 text-green-700 hover:bg-green-100 rounded text-[12.5px] font-bold transition-colors">+ Asset</button>
                    <button onClick={() => setModalState({ type: 'addSupplier', deptId: dept.id })} className="px-2 py-1 bg-orange-50 text-orange-700 hover:bg-orange-100 rounded text-[12.5px] font-bold transition-colors">+ Supplier</button>
                    <button onClick={() => setModalState({ type: 'editDept', deptId: dept.id })} className="p-1.5 text-slate-400 hover:text-[#D4AF37] rounded transition-colors" title="Edit"><Edit2 className="w-3.5 h-3.5" /></button>
                  ...
                  <div className="bg-slate-50/50 px-4 pb-4 pt-1 space-y-4 border-t border-slate-100">
                    ...
                    {deptAssets.length > 0 && (
                      <div className="ml-6 space-y-2">
                        <p className="text-[12px] font-bold text-slate-400 uppercase tracking-widest pl-2 mb-1">Own Assets</p>
                        {deptAssets.map((asset: any) => {
                          const aExp = expandedAssets[asset.id];
                          const piiRecs = asset.piiRecords || [];
                          return (
                            <div key={asset.id} className="bg-white border border-[#D4AF37]/35 rounded-lg">
                              <div className="flex items-center gap-2.5 px-3 py-2.5 hover:bg-slate-50 transition-colors">
                                ...
                                <span className="flex-1 text-[14.5px] font-medium text-slate-800">{asset.name}</span>
                                <span className="text-[12px] px-1.5 py-0.5 bg-slate-100 text-slate-500 rounded">{asset.assetType?.replace(/_/g,' ')}</span>
                                {piiRecs.length === 0 && <span className="flex items-center gap-1 text-[12px] text-amber-600 px-2"><AlertTriangle className="w-3 h-3" /> No PII</span>}
                                <div className="flex items-center gap-1 ml-2">
                                  <button onClick={() => setModalState({ type: 'addPII', deptId: dept.id, assetId: asset.id })} className="px-2 py-1 bg-red-50 text-red-700 hover:bg-red-100 rounded text-[12px] font-bold transition-colors">+ PII</button>
                                  <button onClick={() => setModalState({ type: 'editAsset', deptId: dept.id, assetId: asset.id })} className="p-1 text-slate-400 hover:text-[#D4AF37] rounded" title="Edit Asset"><Edit2 className="w-3 h-3" /></button>
                              ...
                              {aExp && (
                                <div className="border-t border-slate-100 bg-slate-50/50 p-2 space-y-1">
                                  {piiRecs.length === 0 ? <p className="text-[13px] text-slate-400 italic px-2 py-1">No PII records added.</p> : piiRecs.map((pii: any) => (
                                    <div key={pii.id} className="flex items-center gap-2 pl-8 pr-2 py-1.5 text-[13.5px] hover:bg-white rounded transition-colors group">
                                      ...
                                      <span className="text-[12px] px-1.5 py-0.5 bg-white border border-[#D4AF37]/35 text-slate-500 rounded">{pii.sensitivity}</span>
                                      <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity ml-2">
                                        <button onClick={() => setModalState({ type: 'editPII', deptId: dept.id, assetId: asset.id, piiId: pii.id })} className="p-1 text-slate-400 hover:text-[#D4AF37] rounded"><Edit2 className="w-3 h-3" /></button>
                    ...
                    {deptSuppliers.length > 0 && (
                      <div className="ml-6 space-y-2 mt-4">
                        <p className="text-[12px] font-bold text-slate-400 uppercase tracking-widest pl-2 mb-1">Suppliers</p>
                        {deptSuppliers.map((supp: any) => {
                          const sExp = expandedSuppliers[supp.id];
                          const suppAssets = supp.supplierAssets || [];
                          return (
                            <div key={supp.id} className="bg-white border border-[#D4AF37]/35 rounded-lg">
                              <div className="flex items-center gap-2.5 px-3 py-2.5 hover:bg-slate-50 transition-colors">
                                ...
                                <span className="flex-1 text-[14.5px] font-medium text-slate-800">{supp.name}</span>
                                <span className="text-[12px] px-1.5 py-0.5 bg-slate-100 text-slate-500 rounded">{supp.supplierType}</span>
                                <div className="flex items-center gap-1 ml-2">
                                  <button onClick={() => setModalState({ type: 'addSuppAsset', deptId: dept.id, suppId: supp.id })} className="px-2 py-1 bg-amber-50 text-amber-700 hover:bg-amber-100 rounded text-[12px] font-bold transition-colors">+ Vendor Asset</button>
                                  <button onClick={() => setModalState({ type: 'editSupplier', deptId: dept.id, suppId: supp.id })} className="p-1 text-slate-400 hover:text-[#D4AF37] rounded"><Edit2 className="w-3 h-3" /></button>
                              ...
                              {sExp && (
                                <div className="border-t border-slate-100 bg-slate-50/50 p-2 space-y-1">
                                  {suppAssets.length === 0 ? <p className="text-[13px] text-slate-400 italic px-2 py-1">No vendor assets added.</p> : suppAssets.map((sa: any) => (
                                    <div key={sa.id} className="flex items-center gap-2 pl-8 pr-2 py-1.5 text-[13.5px] hover:bg-white rounded transition-colors group">
                                      ...
                                      <span className="text-[12px] px-1.5 py-0.5 bg-white border border-[#D4AF37]/35 text-slate-500 rounded">{sa.assetType?.replace(/_/g,' ')}</span>
                                      <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity ml-2">
                    ...
                    {deptAssets.length === 0 && deptSuppliers.length === 0 && (
                      <p className="text-[13.5px] text-slate-400 italic py-2 pl-6">No assets or suppliers added to this department yet.</p>
                    )}
                  </div>
                )}
              </div>
      ...
      {modalState?.type === 'editAsset' && (
        <Modal title="Edit Asset" onClose={() => setModalState(null)}>
          <AssetForm initial={(depts.find(d => d.id === modalState.deptId)?.assets.find((a: any) => a.id === modalState.assetId) as any) || {}} existingNames={[]} onCancel={() => setModalState(null)}
            onSave={(data) => {
      ...
      {modalState?.type === 'editSupplier' && (
        <Modal title="Edit Supplier" onClose={() => setModalState(null)}>
          <SupplierForm initial={(depts.find(d => d.id === modalState.deptId)?.suppliers.find((s: any) => s.id === modalState.suppId) as any) || {}} onCancel={() => setModalState(null)}
            onSave={(data) => {
