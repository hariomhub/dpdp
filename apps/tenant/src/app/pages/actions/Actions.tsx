  ...
  return (
    <div>
      ...
      {isCO && rejected.length > 0 && (
        <div className="mb-3 p-3 bg-red-50 border border-red-200 rounded-lg">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-[14.5px] font-semibold text-red-700">Requires Your Decision ({rejected.length} action{rejected.length > 1 ? 's' : ''} rejected)</span>
          </div>
          {rejected.map(r => (
            <div key={r.id} className="flex items-center justify-between p-2.5 bg-white border border-red-200 rounded mt-1.5">
              <span className="text-[14.5px] text-slate-800">{r.title}</span>
      ...
      <div className="overflow-x-auto mb-3">
        <div className="flex border-b border-[#D4AF37]/35 min-w-max">
          {ALL_STATUSES.map(s => (
            <button key={s} onClick={() => setActiveTab(s)}
              className={`px-3.5 py-2 text-[14px] font-medium whitespace-nowrap border-b-2 -mb-px transition-colors
                ${activeTab === s ? 'text-[#1A3E5C] border-[#1A3E5C]' : 'text-slate-500 border-transparent hover:text-slate-800'}`}>
              {s}
            </button>
          ))}
        </div>
        ...
        <DataTable headers={['Title', 'Asset', 'Assessment', 'Priority', 'Assignee', 'Due Date', 'Status', 'Updated', '']}>
          {filtered.map(action => (
            <TR key={action.id} onClick={() => navigate(`/org/actions/${action.id}`)}>
              <TD>
                <div>
                  <p className="text-[14.5px] font-medium text-slate-800">{action.title}</p>
                  <p className="text-[13px] text-slate-400 mt-0.5">{action.controlName}</p>
                </div>
              </TD>
              <TD>
                <span className="text-[14px] text-slate-500">{action.assetName}</span>
              </TD>
              <TD>
                <span className="text-[13px] text-slate-400 max-w-[120px] truncate block">Q1 2025 Assessment</span>
              </TD>
              <TD><PriorityChip priority={action.priority} /></TD>
              <TD>
                <span className="text-[14px] text-slate-600">{action.assignee}</span>
              </TD>
              <TD>
                <p className="text-[14px] text-slate-700">{action.dueDate}</p>
              </TD>
              <TD><StatusChip status={action.status} /></TD>
              <TD><span className="text-[13px] text-slate-400">2 days ago</span></TD>
              <TD>
